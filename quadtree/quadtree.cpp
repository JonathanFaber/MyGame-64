...
Quad::Quad(double3 pos, double3 posSquare, double length, char side, double squarePosSide) {
    this->pos = pos;
...
void Quad::update() {
    ...
    if (subdivide && child[0] == nullptr) {
        makeChildren();
    } else if (subdivide && child[0] != nullptr) {
        for (int i = 0; i < 4; i++) {
            child[i]->update();
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        if (child[i] != nullptr) {
            if (child[i]->combine && child[i]->child[0] != nullptr) {
                child[i]->update();
            } else if (child[i]->combine && child[i]->child[0] == nullptr) {
                delete child[i];
                child[i] = nullptr;
            }
        }
...
void Quad::transform(double3 camPos) {
...
void Quad::drawTerrain(bool drawClose) {
    ...
    if (draw && distance < eyeRange) {
        if (dotProduct(posAway, camDir) > 0.5 || distance < quadData[i].length * 2.0) {
