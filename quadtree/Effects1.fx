...
MODEL_VS_OUTPUT MODEL_VS(float4 inPos : POSITION, float2 inTexCoord : TEXCOORD, float3 normal : NORMAL, float3 tangent : TANGENT, float3 bitangent : BITANGENT)
{
	MODEL_VS_OUTPUT output;
...
float4 MODEL_PS(MODEL_VS_OUTPUT input) : SV_TARGET
{
	//input.normal = normalize(input.normal);
