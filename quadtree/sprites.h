...
class text {
	...
	public:
		...
		void update(XMFLOAT2 letter_texCoord) {
			vertices[0] = SpriteVertex(-0.5f, -0.5f, 0.1f, 0.0f + letter_texCoord.x + 0.0001f, 1.0f / 16 + letter_texCoord.y - 0.0001f);
			vertices[1] = SpriteVertex(0.5f, -0.5f, 0.1f, 1.0f / 16 + letter_texCoord.x - 0.0001f, 1.0f / 16 + letter_texCoord.y - 0.0001f);
			vertices[2] = SpriteVertex(0.5f, 0.5f, 0.1f, 1.0f / 16 + letter_texCoord.x - 0.0001f, 0.0f + letter_texCoord.y + 0.0001f);
			vertices[3] = SpriteVertex(-0.5f, 0.5f, 0.1f, 0.0f + letter_texCoord.x + 0.0001f, 0.0f + letter_texCoord.y + 0.0001f);
			...
			memcpy(updatedVertexBufferData.pData, vertices, sizeof(vertices));
	...
	character letters[256];

	void create() {
		for (int i = 0; i < 256; i++)
			letters[i].create(spriteTexCoord_Space);
	}

	void update(std::string message, XMFLOAT2 pos, XMFLOAT2 scale) {
		char charString[1024];
		strcpy_s(charString, message.c_str());

		for (int i = 0; i < 256; i++) {
			int letterID = charString[i];
			...
			letters[i].spriteScale = scale;
			letters[i].spritePos = XMFLOAT2(float(i)*letters[i].spriteScale.x + pos.x, pos.y);

			if (i < message.size())
				letters[i].update(spriteTexCoord);
			else
				letters[i].update(spriteTexCoord_Space);
		}
	}
