...
void CleanUp()
{
	...
	for (int i = 0; i < 32; i++){
		menuText_resume[i].cleanUp();
		menuText_quitWorld[i].cleanUp();
	}
...
bool InitScene()
{
	...
	for (int i = 0; i < 8; i++)
		FPS[i].create(spriteTexCoord_Num[i]);

	for (int i = 0; i < 32; i++){
		menuText_resume[i].create(spriteTexCoord_Box);
		menuText_quitWorld[i].create(XMFLOAT2(0.0f/16, 5.0f/16));
	}
	...
	planetCamProjection = XMMatrixPerspectiveFovLH( 0.4f*3.14f, float(Width)/float(Height), 1000.0f, 1000000000.0f);
...
void UpdateScene(double time)
{
	...
	if(timeSinceFpsCalc > 1.0f){
		...
		std::string stringFPS = boost::lexical_cast<std::string>(fpsCounter);
		char FPSchar[1024];
		strcpy_s(FPSchar, stringFPS.c_str());

		for (int i = 0; i < 8; i++){
			int letter = FPSchar[i];
			XMFLOAT2 spriteTexCoord;

			counter = 0;
			for (int y = 0; y < 16; y++){
				for (int x = 0; x < 16; x++){
					if (counter == letter){
						spriteTexCoord = XMFLOAT2(float(x)/16, float(y)/16);
					}
					counter++;
				}
			}

			FPS[i].spriteScale = XMFLOAT2(24.0f, 24.0f);
			FPS[i].spritePos = XMFLOAT2(float(i)*FPS[i].spriteScale.x - 750.0f, 350.0f);

			if(i < stringFPS.size())
				FPS[i].update(spriteTexCoord);
			else
				FPS[i].update(spriteTexCoord_Space);
		}
		fpsCounter = 0;
	}

	if (showPause == true){
		std::string string1 = "Resume Game";
		char char1[1024];
		strcpy_s(char1, string1.c_str());
		...
		for (int i = 0; i < 32; i++){
			...
			menuText_resume[i].spriteScale = XMFLOAT2(40.0f, 40.0f);
			menuText_resume[i].spritePos = XMFLOAT2(float(i)*menuText_resume[i].spriteScale.x - 180.0f, 200.0f);

			if(i < string1.size())
				menuText_resume[i].update(spriteTexCoord);
			else
				menuText_resume[i].update(spriteTexCoord_Space);
		}
	}
	...
	if (showPause == true){
		...
		for (int i = 0; i < 32; i++){
			...
			menuText_quitWorld[i].spriteScale = XMFLOAT2(40.0f, 40.0f);
			menuText_quitWorld[i].spritePos = XMFLOAT2(float(i)*menuText_quitWorld[i].spriteScale.x - 180.0f, -200.0f);

			if(i < string1.size())
				menuText_quitWorld[i].update(spriteTexCoord);
			else
				menuText_quitWorld[i].update(spriteTexCoord_Space);
		}
	}
...
void DrawScene()
{
	...
	if (showPause == true) {
		for (int i = 0; i < 32; i++) {
			menuText_resume[i].draw();
			menuText_quitWorld[i].draw();
		}
