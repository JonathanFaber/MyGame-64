...
InstanceData grassPos[10000];
int numGrassPatches = 10000;
...
void updateGrass(double3 camPos, double timeElaps) {
	...
	for (int x = -50; x < 50; x++) {
		for (int z = -50; z < 50; z++) {
			//if (float(allPoints[x+int(floor(camPos.x)-BigChunkSize*region.x)+512][z+int(floor(camPos.z)-BigChunkSize*region.z)+512]) > -0.5f){
			if (vLength(double3(double(x), camPos.y - allPoints[x + int(floor(camPos.x) - BigChunkSize*region.x) + 512][z + int(floor(camPos.z) - BigChunkSize*region.z) + 512], double(z))) < 100.0f) {
				//if (allSmoothBiomePoints[x + int(floor(camPos.x))-biomeRegion.x*BiomeChunkSize+BiomeChunkSize][z + int(floor(camPos.z))-biomeRegion.z*BiomeChunkSize+BiomeChunkSize]){	
				windSpeed = sin((sin(timeElaps + (x + floor(camPos.x)) + (z + floor(camPos.z)))) * windIntensity);

				grassPos[counter].instancePos = XMFLOAT2(float(x) + floor(camPos.x), float(z) + floor(camPos.z));
				grassPos[counter].instanceHeights.x = allPoints[x + int(floor(camPos.x) - BigChunkSize*region.x) + 512][z + int(floor(camPos.z) - BigChunkSize*region.z) + 512];
				grassPos[counter].instanceHeights.y = allPoints[x + int(floor(camPos.x) - BigChunkSize*region.x) + 513][z + int(floor(camPos.z) - BigChunkSize*region.z) + 512];
				grassPos[counter].instanceHeights.z = allPoints[x + int(floor(camPos.x) - BigChunkSize*region.x) + 513][z + int(floor(camPos.z) - BigChunkSize*region.z) + 513];
				grassPos[counter].instanceHeights.w = allPoints[x + int(floor(camPos.x) - BigChunkSize*region.x) + 512][z + int(floor(camPos.z) - BigChunkSize*region.z) + 513];
